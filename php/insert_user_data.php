<?php
  ...
  // CRUD : Created, Read, Updae, Delete
...
//   echo var_dump($db_conn);
  ...
  $sql = "SELECT user_name FROM spl_user";
  $sql = "SELECT * FROM spl_user";

  // 3. 데이터 조회 필터링 : SELECT [field name] FROM [table name] WEHRE [field name] = [field value]
  $sql = "SELECT * FROM spl_user WHERE user_idx=2";

  // 4. 데이터 정렬 : SELECT [field name] FROM [table name] ORDER BY [field name] DESC(연순) or ASC (정순))
  $sql = "SELECT * FROM spl_user ORDER BY user_idx DESC";

  // 5. 데이터  개수 지정 조회 : SELECT [field name] FROM [table name] LIMIT [number][start index],[number][length]]
  ...
  // WHERE   필터링 없이 업데이트만 한다면
  $sql = "UPDATE spl_user SET user_name='babo' WHERE user_idx=2";
  
  // 7. 데이터 삭제 (delete) : DELETE FROM [table name] WHERE [field name]=[field value];
  $sql="DELETE FROM spl_user WHERE user_idx=2 and user_name='babo'"
